...
public class SpoonCallGraph {
    ...
    private static boolean includeLocationOfAccess = false;

    private static String sourcesPath = null;
    private static String repoURL = null;
    private static String projectNameInput = null;
    private static String microservicePathRegex = null;
    private static String dataClassFqnRegexes = null;
    ...
    public static void main(String[] args) throws GitAPIException, IOException {
        if (args.length == 0) {
            ...
            // Launcher Choice, Sources Choice, ORM Choice, projectName, path/link, dataClassFqnRegexes, microserviceFqnRegex
            ...
            sourcesPath = args[4];
            if (args.length > 5) {
                microservicePathRegex = args[5];
                if (args.length == 7) {
                    dataClassFqnRegexes = args[6];
                }
            }
        } else {
        ...
        switch (ormChoice) {
            case Constants.FENIX_FRAMEWORK:
                collector = new FenixFrameworkCollector(launcherChoice, projectName, file.getAbsolutePath(), includeLocationOfAccess);
                break;
            case Constants.SPRING_DATA_JPA:
                collector = new SpringDataJPACollector(
                        launcherChoice,
                        projectName,
                        file.getAbsolutePath(),
                        microservicePathRegex,
                        dataClassFqnRegexes,
                        includeLocationOfAccess
                );
                break;
            default:
                System.exit(1);
    ...
    private static void showDialogSecond() {
        ...
        JTextField microservicePathRegexField = new JTextField(5);
        JTextField dataClassFqnRegexesField = new JTextField(5);
        ...
        panel.add(new JLabel("Microservice path regex:"));
        panel.add(microservicePathRegexField);

        panel.add(new JLabel("Data class FQN regexes, seperated by comma (optional):"));
        panel.add(dataClassFqnRegexesField);
        ...
        if (option == JOptionPane.OK_OPTION) {
            ...
            microservicePathRegex = microservicePathRegexField.getText();
            dataClassFqnRegexes = dataClassFqnRegexesField.getText();
    ...
    private static void showDialogFirst() {
        ...
        JCheckBox includeLocationWithAccesses = new JCheckBox();
        ...
        panel.add(new JLabel("Include location of entity accesses"));
        panel.add(includeLocationWithAccesses);
        ...
        if (option == JOptionPane.OK_OPTION) {
            ...
            includeLocationOfAccess = includeLocationWithAccesses.isSelected();
        }
        else {
            System.exit(1);
        }
