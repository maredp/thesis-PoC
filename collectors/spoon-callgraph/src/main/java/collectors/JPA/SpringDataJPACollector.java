...
public class SpringDataJPACollector extends SpoonCollector {
    ...
    private String microservicePathRegex;
    private  List<String> dataClassFqnRegexes;

    public SpringDataJPACollector(
            int launcherChoice,
            String repoName,
            String projectPath,
            String microservicePathRegex,
            String dataClassFqnRegexes,
            boolean includeLocationOfAccess
    ) throws IOException {
        super(launcherChoice, repoName, projectPath, includeLocationOfAccess);
        ...
        this.microservicePathRegex = "";
        this.dataClassFqnRegexes = new ArrayList<>();

        if (microservicePathRegex != null) {
            this.microservicePathRegex = microservicePathRegex;
        }
        if (dataClassFqnRegexes != null) {
            this.dataClassFqnRegexes = new ArrayList<>(Arrays.asList(dataClassFqnRegexes.split(",")));
        }
    }
            ...
            if (existsAnnotation(clazzAnnotations, "Controller") ||
                ...
                if (clazz.isInterface() || clazz.isAbstract()) {
                    abstractClassesAndInterfacesImplementorsMap.put(clazz.getSimpleName(), new ArrayList<>());
                        ...
                        existsAnnotation(clazzAnnotations, "Entity") ||
                        existsAnnotation(clazzAnnotations, "MappedSuperclass") ||
                        existsAnnotation(clazzAnnotations, "Immutable") ||
                        fqnMatches(clazz, dataClassFqnRegexes)
                ) {
                    String clusterName = findFirstFqnMatch(clazz, microservicePathRegex);
                    clusterName = clusterName == null ? "monolith" : clusterName;
                    addClusterEntity(clusterName, clazz.getSimpleName());
    ...
    *  isRepository: true if class Package = org.springframework.data.repository or if it extends (in)directly from it */
        ...
        callerMethod.accept(new CtScanner() {
                            ...
                            if (target != null) {
                                ...
                                if (executableDeclaration != null) {
                                    ...
                                    if (eDM.isAbstract()) {
                                        ...
                                        List<CtMethod> explicitImplementationsOfAbstractMethod = getExplicitImplementationsOfAbstractMethodJPA(eDM, methodStack.toString());
                    ...
                    if (calleeLocation.getExecutable().getDeclaringType() == null)
                        ...
                        if (calleeLocation instanceof CtInvocation) {
                            ...
                            if (collectionEntityAccess && !collectionTransientField) {
                                registerDomainAccess(collectionDeclaringTypeName, collectionFieldAccessedType, (CtInvocation) calleeLocation, methodStack.toString());
                            }
                        }
                    }
            ...
            public <T> void visitCtMethod(CtMethod<T> m) {
                ...
                * until we find a non-abstract implementor */
                if (m.isAbstract()) {
                    try {
                        List<CtMethod> explicitImplementationsOfAbstractMethod = getExplicitImplementationsOfAbstractMethodJPA(m, methodStack.toString());
                        for (CtMethod ctMethod : explicitImplementationsOfAbstractMethod) {
                            if (!methodStack.contains(ctMethod.getPosition())) {
                ...
                try {
                    ...
                    * im saving the prevCalleeLocation */
                    ...
                    if (allDomainEntities.contains(declaringTypeName)) {
                        ...
                        addEntitiesSequenceAccess(declaringTypeName, mode, methodStack.toString());
                            ...
                            if (actualTypeArguments.size() > 0) {
                                for (CtTypeReference ctTypeReference : actualTypeArguments) {
                                    if (allDomainEntities.contains(ctTypeReference.getSimpleName())) {
                                        addEntitiesSequenceAccess(ctTypeReference.getSimpleName(), mode, methodStack.toString());
                                    }
                                }
                            }
                            else {
                                if (allDomainEntities.contains(fieldType.getSimpleName())) {
                                    addEntitiesSequenceAccess(fieldType.getSimpleName(), mode, methodStack.toString());
                                }
                            }
                        }
                    }
    ...
    private void analyzeCallAndRegisterAccess(
            CtExecutableReference executable,
            CtType repositoryCtType,
            String location) throws UnkownMethodException {
        // Entity Access Call
        ...
        for (CtMethod ctM : methodsByName) {
            if (isEqualMethodSignature(executable, ctM.getReference())) {
                ...
                declared = true;
                registerRepositoryAccess(repositoryCtType.getSimpleName(), ctM.getReference(), location);
                break;
            }
        }
        if (!declared) {
            registerSpringDataRepositoryAccess(repositoryCtType.getSimpleName(), executable.getSimpleName(), location);
        }
        return;
    }
    ...
    private List<CtMethod> getExplicitImplementationsOfAbstractMethodJPA(CtMethod abstractMethod, String location) throws UnkownMethodException {
        ...
        if (implementors != null) {
            for (CtType ctImplementorType : implementors) {
                if (isProjectDeclaredRepository(ctImplementorType.getSimpleName())) {
                    analyzeCallAndRegisterAccess(abstractMethod.getReference(), ctImplementorType, location);
                }
                else {
    ...
    * */
    private void registerDomainAccess(
            String declaringTypeName,
            CtTypeReference fieldAccessedType,
            CtInvocation calleeLocation,
            String location) {
        ...
        // class access
        addEntitiesSequenceAccess(declaringTypeName, mode, location);
        // field access
        if (mode.equals("R")) {
            ...
            if (actualTypeArguments.size() > 0) {
                for (CtTypeReference ctTypeReference : actualTypeArguments) {
                    if (allDomainEntities.contains(ctTypeReference.getSimpleName())) {
                        addEntitiesSequenceAccess(ctTypeReference.getSimpleName(), mode, location);
                    }
                }
            }
            else {
                if (allDomainEntities.contains(returnType.getSimpleName())) {
                    addEntitiesSequenceAccess(returnType.getSimpleName(), mode, location);
                }
            }
        }
            ...
            for (String argTypeName : argTypeNames) {
                if (allDomainEntities.contains(argTypeName)) {
                    addEntitiesSequenceAccess(argTypeName, mode, location);
                }
            }
        }
    }
    ...
    private void registerRepositoryAccess(
            String repositorySimpleName,
            CtExecutableReference methodDeclaration,
            String location) {
        ...
        if (queryAnnotation != null) {
            ...
            if (!namedQueryName.equals("")) {
                if (!value.equals("")) {
                    ...
                    if (q == null) {
                        ...
                        if (q.isNative())
                            parseNativeQuery(q.getValue(), location);
                        else
                            parseHqlQuery(q.getValue(), repositorySimpleName, location);
                    }
                }
            }

            if (nativeQuery) {
                parseNativeQuery(value, location);
            }
            else { // HQL query
                parseHqlQuery(value, repositorySimpleName, location);
            }
        }
            ...
            if (q == null) {
                registerSpringDataRepositoryAccess(methodDeclaration.getDeclaringType().getSimpleName(), methodDeclaration.getSimpleName(), location);
            }
            else {
                if (q.isNative())
                    parseNativeQuery(q.getValue(), location);
    ...
    private void parseNativeQuery(String sql, String location) {
        try {
            ...
            for (QueryAccess qa : accesses) {
                ...
                for (String typeName : classes.getListOfClasses()) {
                    ...
                    if (mode == null)
                        mode = "R";
                    addEntitiesSequenceAccess(typeName, mode, location);
                }
            }
        } catch (Exception e) {
            System.err.println("Exception on query: \"" + sql + "\"");
    ...
    private void parseHqlQuery(String hql, String repositorySimpleName, String location) {
        try {
...
//                    System.err.println("HQL Parser: Couldn't map class " + split[0]);
                    ...
                    addEntitiesSequenceAccess(clazz.getSimpleName(), a.getMode(), location);
                            ...
                            if (!allEntities.contains(field.getType().getSimpleName()) && actualTypeArguments.size() > 0) {
                                for (CtTypeReference ctTypeReference : actualTypeArguments) {
                                    ...
                                    if (allDomainEntities.contains(processedType.getSimpleName())) {
                                        addEntitiesSequenceAccess(processedType.getSimpleName(), a.getMode(), location);
                                ...
                                if (fieldTypeReference instanceof CtTypeParameterReference) {
                                    ...
                                    if (allDomainEntities.contains(boundingType.getSimpleName())) {
                                        addEntitiesSequenceAccess(boundingType.getSimpleName(), a.getMode(), location);
                                        clazz = boundingType.getTypeDeclaration();
                                    }
                                }

                                else if (allDomainEntities.contains(fieldTypeReference.getSimpleName())) {
                                    addEntitiesSequenceAccess(fieldTypeReference.getSimpleName(), a.getMode(), location);
    ...
    private void registerSpringDataRepositoryAccess(String repositoryClassName, String methodName, String location) {
        String mode;
        // Read Access
        ...
        for (Repository r : repositories) {
            if (r.getRepositoryClassName().equals(repositoryClassName)) {
                ...
                addEntitiesSequenceAccess(typeClassName, mode, location);
                return;
            }
        }
    }
